...
export function AuthProvider({ children }) {
  ...
  function addFavorites(kaomoji) {
    if (currentUser) {
      // only adds when user is logged in
      ...
      if (!userProfile.favorites.some((k) => k.id === kaomoji.id)) {
  ...
  function removeFavorites(kaomoji) {
    if (currentUser) {
      ...
      newProfile.favorites = newProfile.favorites.filter(
        (item) => item.id !== kaomoji.id
      );
